...
> Tính tiến triển (liveness) và tính an toàn (safety) là những thuộc tính mô tả khả năng của một thuật toán trong việc giải một bài toán cụ thể cũng như tính đúng đắn của kết quả nó tạo ra. Một cách hình thức hơn, tính tiến triển là thuộc tính bảo đảm rằng một sự kiện dự định cụ thể nhất định phải xảy ra. Ví dụ, nếu một trong các tiến trình đã bị lỗi, bộ phát hiện lỗi phải phát hiện được lỗi đó. Tính an toàn bảo đảm rằng những sự kiện ngoài ý muốn sẽ không xảy ra. Ví dụ, nếu một bộ phát hiện lỗi đã đánh dấu một tiến trình là đã chết, thì trên thực tế tiến trình đó phải thực sự đã chết [LAMPORT77] [RAYNAL99] [FREILING11] .
...
respond to the message, so P 1 proceeds by selecting multiple random members ( P 3 and P 4 ). These random members try sending heartbeat messages to P 2 and, if it responds, forward acknowledgments back to P .

> phản hồi thông điệp này, nên P1 tiếp tục bằng cách chọn ra nhiều thành viên ngẫu nhiên ( P3 và P4 ). Các thành viên ngẫu nhiên này thử gửi các thông điệp nhịp tim tới P2 và, nếu nó phản hồi, sẽ chuyển tiếp các xác nhận trở lại cho P1 .
...
This allows accounting for both direct and indirect reachability. For example, if we have processes P 1 , P 2 , and P 3 , we can check the state of P 3 from the perspective of both P and P .

> Điều này cho phép tính đến cả khả năng tiếp cận trực tiếp lẫn gián tiếp. Ví dụ, nếu ta có các tiến trình P1 , P2 và P3 , ta có thể kiểm tra trạng thái của P3 từ góc nhìn của cả P1 lẫn P2 .
...
- d) Eventually, P 1 and P 3 notice that both P 1 and P 2 do not respond, and process failure propagates to the entire group.

> - d) Rốt cuộc, P1 và P3 nhận thấy rằng cả P1 lẫn P2 đều không phản hồi, và sự cố tiến trình lan ra toàn bộ nhóm.
