...
Figure 12-3 (adapted from [GONÇALVES15] ) shows an example of the state representation of three nodes in the system, P 1 , P 2 and P 3 , from the perspective of P 2 , tracking which values it has seen. Each time P 2 makes a write or receives a replicated value, it updates this table.

> Hình 12-3 (phỏng theo [GONÇALVES15]) thể hiện một ví dụ về cách biểu diễn trạng thái của ba nút trong hệ thống, P1, P2 và P3, từ góc nhìn của P2, theo dõi những giá trị nào nó đã thấy. Mỗi lần P2 thực hiện một lần ghi hoặc nhận một giá trị được nhân bản, nó cập nhật bảng này.
...
from the node identifier to a pair of latest values, up to which it has seen consecutive writes, and a bitmap where other seen writes are encoded as 1 . (3, 01101 2 ) here means that node P has seen consecutive updates up to the third value, and it has seen

> từ định danh nút tới một cặp gồm giá trị mới nhất, tính đến đó nó đã thấy các lần ghi liên tiếp, và một bitmap trong đó các lần ghi khác đã thấy được mã hóa bằng 1. Ở đây (3, 01101 2) nghĩa là nút P đã thấy các cập nhật liên tiếp đến tận giá trị thứ ba, và nó đã thấy
...
The active view is updated depending on the state changes of nodes in this view and requests from peers. If a process P 1 suspects that P 2 , one of the peers from its active view, has failed, P 1 removes P 2 from its active view and attempts to establish a connection with a replacement process P from the passive view. If the connection fails, P is

> Góc nhìn chủ động được cập nhật tùy theo các thay đổi trạng thái của những nút trong góc nhìn này và các yêu cầu từ các nút ngang hàng. Nếu một tiến trình P1 nghi ngờ rằng P2, một trong các nút ngang hàng thuộc góc nhìn chủ động của nó, đã bị lỗi, P1 loại P2 khỏi góc nhìn chủ động của mình và cố thiết lập kết nối với một tiến trình thay thế P lấy từ góc nhìn thụ động. Nếu kết nối thất bại, P bị
...
the connection if its active view is already full. If P 1 ’s view is empty, P 3 has to replace one of its current active view peers with P 1 . This helps bootstrapping or recovering nodes to quickly become effective members of the cluster at the cost of cycling some connections.

> kết nối nếu góc nhìn chủ động của nó đã đầy. Nếu góc nhìn của P1 rỗng, P3 phải thay thế một trong các nút ngang hàng thuộc góc nhìn chủ động hiện tại của nó bằng P1. Điều này giúp các nút đang khởi động hoặc đang phục hồi nhanh chóng trở thành thành viên hữu hiệu của cụm, với cái giá là phải luân chuyển một số kết nối.
