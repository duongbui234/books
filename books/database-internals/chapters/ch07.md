...
> - B-Tree sao-khi-ghi (copy-on-write) được tổ chức giống B-Tree, nhưng các nút của chúng là bất biến và không được cập nhật tại chỗ. Thay vào đó, các trang được sao chép, cập nhật, rồi ghi ra vị trí mới.
> - Lazy B-Tree giảm số yêu cầu I/O sinh ra từ những lần ghi liên tiếp vào cùng một nút bằng cách đệm các cập nhật cho nút. Trong chương tiếp theo, chúng ta cũng sẽ bàn về LSM Tree hai thành phần (xem "Two-component LSM Tree" ở trang 132), vốn đẩy việc đệm đi xa thêm một bước để hiện thực các B-Tree hoàn toàn bất biến.
> - FD-Tree áp dụng một cách tiếp cận khác cho việc đệm, phần nào giống LSM Tree (xem "LSM Trees" ở trang 130). FD-Tree đệm các cập nhật trong một B-Tree nhỏ. Ngay khi cây này đầy, nội dung của nó được ghi vào một run bất biến. Các cập nhật lan truyền giữa các tầng run bất biến theo kiểu thác đổ, từ tầng cao xuống tầng thấp.
> - Bw-Tree tách các nút B-Tree thành nhiều phần nhỏ hơn được ghi theo cách chỉ-nối-thêm. Điều này giảm chi phí của những lần ghi nhỏ bằng cách gom các cập nhật cho những nút khác nhau lại thành lô.
> - Cache-oblivious B-Tree cho phép xử lý các cấu trúc dữ liệu trên đĩa theo cách rất giống với cách chúng ta xây dựng các cấu trúc trong bộ nhớ.
...
> 1. Tách —Một nút delta tách đặc biệt được nối thêm vào nút đang tách để báo cho các bên đọc biết về lượt tách đang diễn ra. Nút delta tách giữ một khóa phân tách tại điểm giữa để vô hiệu hóa các bản ghi trong nút đang tách, cùng một liên kết tới nút anh em logic mới.
> 2. Cập nhật nút cha —Tại thời điểm này, tình huống tương tự lượt nửa-tách của Blink-Tree (xem "Blink-Trees" ở trang 107), vì nút đã truy cập được thông qua con trỏ nút delta tách, nhưng chưa được nút cha tham chiếu tới, và các bên đọc phải đi qua nút cũ rồi men theo con trỏ anh em để tới được nút anh em vừa tạo. Một nút mới được thêm làm con của nút cha, để các bên đọc có thể tới thẳng nó thay vì bị chuyển hướng qua nút đang tách, và lượt tách hoàn tất.
...
> 1. Gỡ nút anh em —Một nút delta gỡ đặc biệt được tạo ra và nối thêm vào nút anh em bên phải, báo hiệu bắt đầu SMO gộp và đánh dấu nút anh em bên phải để xóa.
> 2. Gộp —Một nút delta gộp được tạo trên nút anh em bên trái để trỏ tới nội dung của nút anh em bên phải, biến nội dung đó thành một phần logic của nút bên trái.
> 3. Cập nhật nút cha —Tại thời điểm đó, nội dung của nút anh em bên phải đã truy cập được từ nút bên trái. Để kết thúc tiến trình gộp, liên kết tới nút anh em bên phải phải được gỡ khỏi nút cha.
