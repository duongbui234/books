...
> Giống như các sách và bài báo khác về chủ đề này, chúng ta dùng B-Tree làm ví dụ tiêu biểu cho cấu trúc khả biến và Log-Structured Merge Tree (LSM Tree) làm ví dụ cho cấu trúc bất biến. LSM Tree bất biến dùng lưu trữ chỉ-nối-thêm và hòa giải khi gộp, còn B-Tree định vị các bản ghi dữ liệu trên đĩa và cập nhật các trang tại đúng độ dời ban đầu của chúng trong tệp.
...
> 1. Ngay khi tiến trình xả bắt đầu, mọi lần ghi mới đều phải đi vào memtable mới.
> 2. Trong lúc xả cây con, cả cây con nằm trên đĩa lẫn cây con nằm trong bộ nhớ đang được xả đều phải tiếp tục truy cập được cho các lượt đọc.
> 3. Sau khi xả, việc công bố nội dung đã gộp và loại bỏ những nội dung chưa gộp nằm trên đĩa lẫn trong bộ nhớ phải được thực hiện một cách nguyên tử.
