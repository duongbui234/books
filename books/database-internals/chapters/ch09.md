# Part I Conclusion — Part I Kết luận
...
> Khi dùng tính bất biến, nếu không dùng kèm đệm, chúng ta sẽ có được các cấu trúc lưu trữ không sắp thứ tự như Bitcask và WiscKey (ngoại trừ B-Tree sao-khi-ghi, vốn sao chép, sắp xếp lại và di dời các trang của mình). WiscKey chỉ lưu các khóa trong những LSM Tree đã sắp xếp và cho phép truy xuất các bản ghi theo thứ tự khóa nhờ chỉ mục khóa. Trong Bw-Tree, một số nút (những nút đã được hợp nhất) giữ các bản ghi dữ liệu theo thứ tự khóa, còn các nút Bw-Tree logic còn lại có thể có các cập nhật delta nằm rải rác trên nhiều trang khác nhau.
...
# PART II Distributed Systems — PART II Hệ phân tán
