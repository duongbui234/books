...
> Từ góc độ lý thuyết, các thuật toán đồng thuận có ba thuộc tính:
...
any other proposal earlier, it responds with a Promise(m, v accepted ) message, notifying the proposer that it has already accepted the proposal with a sequence number m .

> bất kỳ đề xuất nào khác trước đó, nó phản hồi bằng một thông điệp Promise(m, v accepted), thông báo cho bên đề xuất rằng nó đã chấp nhận đề xuất với số trình tự m.
...
lects a quorum of acceptor responses ( A 1 and A 2 in this case), and proceeds by committing the old value V1 , proposed by P 1 .

> thập một quorum các phản hồi của bên chấp nhận (A1 và A2 trong trường hợp này), và tiến hành bằng cách commit giá trị cũ V1, do P1 đề xuất.
...
> - P bị lỗi hệt như trong ví dụ trước, sau khi chỉ gửi giá trị V1 tới
...
- The next proposer, P 2 , starts a new round with a higher proposal number 2 , and collects a quorum of acceptor responses, but this time A and A are first to

> - Bên đề xuất kế tiếp, P2, bắt đầu một vòng mới với số đề xuất cao hơn 2, và thu thập một quorum các phản hồi của bên chấp nhận, nhưng lần này A và A mới là những bên đầu tiên
...
- Proposer P 2 , which started the round after P 1 failed, does not overlap with A 1 and proceeds to commit its value instead.
- Any proposer that comes after this round that will overlap with A , will ignore A ’s

> - Bên đề xuất P2, vốn đã bắt đầu vòng của mình sau khi P1 bị lỗi, không giao nhau với A1 và thay vào đó tiến hành commit giá trị của chính nó.
> - Bất kỳ bên đề xuất nào đến sau vòng này mà giao nhau với A đều sẽ bỏ qua giá trị của A
...
- P 1 starts with proposal number 1 and no dependencies, and sends a PreAc cept(1, ∅ ) message. Since the command logs of P and P are empty, P can pro‐

> - P1 bắt đầu với số đề xuất 1 và không có phụ thuộc nào, rồi gửi một thông điệp PreAccept(1, ∅). Vì nhật ký lệnh của P và P đều trống, P có thể tiến
...
that point, it also declares no dependencies and sends a PreAccept(2, ∅ ) message. P 4 is not aware of the committed proposal 1 , but P 3 notifies P 5 about the conflict and sends its command log: {1} .

> thời điểm đó, nó cũng khai báo không có phụ thuộc nào và gửi một thông điệp PreAccept(2, ∅). P4 không hay biết về đề xuất 1 đã được commit, nhưng P3 thông báo cho P5 về xung đột và gửi nhật ký lệnh của nó: {1}.

- P updates its local dependency list and sends a message to make sure replicas

> - P cập nhật danh sách phụ thuộc cục bộ của mình và gửi một thông điệp để bảo đảm rằng các bản sao
...
During phase 2 The client notifies all servers about the value it has picked during the first phase by sending them P2 A (register, value) . If the majority of servers respond to this message, it can output the decision value. Otherwise, it starts again from phase 1.

> Trong pha 2 Client thông báo cho tất cả các máy chủ về giá trị mà nó đã chọn trong pha thứ nhất bằng cách gửi cho chúng P2A(register, value). Nếu đa số các máy chủ phản hồi thông điệp này, nó có thể xuất ra giá trị quyết định. Ngược lại, nó bắt đầu lại từ pha 1.
