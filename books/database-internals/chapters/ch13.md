...
> Nhân bản đặc biệt quan trọng trong các triển khai đa trung tâm dữ liệu. Trong trường hợp này, nhân bản theo địa lý (geo-replication) phục vụ nhiều mục đích: nó tăng tính sẵn sàng và khả năng chịu đựng sự cố của một hoặc nhiều trung tâm dữ liệu nhờ cung cấp sự dư thừa. Nó cũng có thể giúp giảm độ trễ bằng cách đặt một bản sao dữ liệu ở vị trí vật lý gần client hơn.
...
- c) The operation executed by P 2 starts after and completes before the operation executed by P . These operations are concurrent , too.

> - b) Có sự chồng lấn giữa hai thao tác, nên các thao tác này là đồng thời .
> - c) Thao tác do P2 thực thi bắt đầu sau và kết thúc trước thao tác do P1 thực thi. Các thao tác này cũng là đồng thời .
...
request was issued, and t 2 , when the process received a response.

> được phát ra, và t2 , khi tiến trình nhận được phản hồi.
...
Even though in wall-clock terms, 1 was written before 2 , it can get ordered after 2 . At the same time, while P 3 already reads the value 1 , P 4 can still read 2 . However, both orders, 1 → 2 and 2 → 1 , are valid, as long as they’re consistent for different readers. What’s important here is that both P and P have observed values in the same order :

> Mặc dù xét theo đồng hồ treo tường, 1 được ghi trước 2 , nó vẫn có thể được sắp thứ tự sau 2 . Đồng thời, trong khi P3 đã đọc được giá trị 1 , P4 vẫn có thể đọc được 2 . Tuy nhiên, cả hai thứ tự, 1 → 2 và 2 → 1 , đều hợp lệ, miễn là chúng nhất quán đối với những bên đọc khác nhau. Điều quan trọng ở đây là cả P3 lẫn P4 đều đã quan sát các giá trị theo cùng một thứ tự :
...
specifies that it is logically ordered after t 1 , requiring operations to propagate only in the order established by the logical clock.

> chỉ định rằng nó được sắp thứ tự logic sau t1 , đòi hỏi các thao tác chỉ được lan truyền theo thứ tự do đồng hồ logic thiết lập.
...
Figure 11-8 shows processes P 1 and P 2 making causally related writes, which propagate to P 3 and P 4 in their logical order. This prevents us from the situation shown in Figure 11-6 ; you can compare histories of P and P in both figures.

> Hình 11-8 cho thấy các tiến trình P1 và P2 thực hiện những lần ghi có quan hệ nhân quả, và chúng lan truyền tới P3 và P4 theo đúng thứ tự logic của chúng. Điều này giúp ta tránh được tình huống thể hiện trong Hình 11-6 ; bạn có thể so sánh lịch sử của P3 và P4 trong cả hai hình.
...
Session models make no assumptions about operations made by different processes (clients) or from the different logical session [TANENBAUM14] . These models describe operation ordering from the point of view of a single process. However, the same guarantees have to hold for every process in the system. In other words, if P 1 can read its own writes, P 2 should be able to read its own writes, too.

> Các mô hình phiên không đưa ra giả định nào về những thao tác được thực hiện bởi các tiến trình (client) khác nhau hoặc từ các phiên logic khác nhau [TANENBAUM14] . Các mô hình này mô tả thứ tự thao tác từ góc nhìn của một tiến trình đơn lẻ. Tuy nhiên, cùng những bảo đảm đó phải đúng với mọi tiến trình trong hệ thống. Nói cách khác, nếu P1 có thể đọc được các lần ghi của chính nó, thì P2 cũng phải đọc được các lần ghi của chính nó.
...
> Khi thực hiện một lần ghi, bên điều phối (coordinator) phải gửi nó tới N nút, nhưng chỉ cần chờ W nút trước khi tiếp tục (hoặc W - 1 trong trường hợp bên điều phối cũng là một replica). Các thao tác ghi còn lại có thể hoàn tất bất đồng bộ hoặc thất bại. Tương tự, khi thực hiện một lần đọc, bên điều phối phải thu thập ít nhất R phản hồi. Một số cơ sở dữ liệu dùng thực thi suy đoán (speculative execution) và gửi thêm các yêu cầu đọc để giảm độ trễ phản hồi của bên điều phối. Điều này nghĩa là nếu một trong các yêu cầu đọc gửi ban đầu bị lỗi hoặc đến chậm, các yêu cầu suy đoán có thể được tính vào R thay cho nó.
