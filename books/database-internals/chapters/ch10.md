...
> - n , cho biết số thứ tự cao nhất mà tính đến đó nó đã thấy toàn bộ

messages. Messages up to this number can be put back in order.

> các thông điệp. Các thông điệp tính đến số này có thể được đưa trở lại đúng thứ tự.

- n processed , specifying the highest sequence number, up to which messages were put back in their original order and processed . This number can be used for deduplication.

> - n processed , cho biết số thứ tự cao nhất mà tính đến đó các thông điệp đã được đưa trở lại thứ tự ban đầu và đã được xử lý . Con số này có thể được dùng để khử trùng lặp.

If the received message has a nonconsecutive sequence number, the receiver puts it into the reordering buffer. For example, it receives a message with a sequence number 5 after receiving one with 3 , and we know that 4 is still missing, so we need to put 5 aside until 4 comes, and we can reconstruct the message order. Since we’re building on top of a fair-loss link, we assume that messages between n consecutive and n max_seen will eventually be delivered.

> Nếu thông điệp nhận được có số thứ tự không liên tiếp, bên nhận đặt nó vào vùng đệm sắp xếp lại (reordering buffer). Ví dụ, nó nhận một thông điệp có số thứ tự 5 sau khi đã nhận thông điệp mang số 3 , và ta biết rằng 4 vẫn còn thiếu, nên ta cần để 5 sang một bên cho tới khi 4 tới, rồi mới có thể tái dựng thứ tự thông điệp. Vì ta đang xây dựng trên nền một kênh fair-loss, ta giả định rằng các thông điệp nằm giữa n consecutive và n max_seen rốt cuộc sẽ được chuyển tới.
