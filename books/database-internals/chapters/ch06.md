...
> Tính cô lập định nghĩa khi nào các thay đổi lên trạng thái cơ sở dữ liệu có thể trở nên hiển thị, và những thay đổi nào có thể trở nên hiển thị với các giao dịch đồng thời. Vì lý do hiệu năng, nhiều cơ sở dữ liệu dùng các mức cô lập yếu hơn định nghĩa vừa nêu về tính cô lập. Tùy vào các phương pháp và cách tiếp cận được dùng cho điều khiển tương tranh, các thay đổi do một giao dịch thực hiện có thể hiển thị hoặc không hiển thị với các giao dịch đồng thời khác (xem "Isolation Levels" ở trang 96).
...
> - Kết nạp (admission) , duy trì các phần tử mới được thêm vào, được hiện thực bằng chính sách LRU.
> - Thử thách (probation) , giữ các phần tử nhiều khả năng bị tống ra nhất.
> - Bảo vệ (protected) , giữ các phần tử sẽ còn ở lại trong hàng đợi lâu hơn.
...
value for the address field, and transaction T 2 reads the updated address before T 1 commits. Transaction T 1 aborts and rolls back its execution results. However, T 2 has already been able to read this value, so it has accessed the value that has never been committed.

> giá trị mới cho trường địa chỉ, và giao dịch T2 đọc địa chỉ đã cập nhật đó trước khi T1 commit. Giao dịch T1 bị abort và cuộn lại các kết quả thực thi của nó. Tuy nhiên, T2 đã kịp đọc giá trị này, nên nó đã truy cập một giá trị chưa bao giờ được commit.
...
this change. If T 1 requests the same row again before finishing its execution, the result will differ from the previous run.

> thay đổi này. Nếu T1 yêu cầu lại chính hàng đó trước khi kết thúc lượt thực thi của mình, kết quả sẽ khác với lần chạy trước.
...
V . T 1 and T 2 read the value of V . T 1 updates V and commts, and T 2 updates V after that and commits as well. Since the transactions are not aware about each other’s existence, if both of them are allowed to commit, the results of T 1 will be overwritten by the results of T , and the update from T will be lost.

> V . T1 và T2 đọc giá trị của V . T1 cập nhật V rồi commit, sau đó T2 cập nhật V và cũng commit. Vì các giao dịch không hay biết về sự tồn tại của nhau, nếu cả hai đều được phép commit, các kết quả của T1 sẽ bị ghi đè bởi các kết quả của T2 , và cập nhật từ T1 sẽ bị mất.
...
A write skew occurs when each individual transaction respects the required invariants, but their combination does not satisfy these invariants. For example, transactions T 1 and T 2 modify values of two accounts A 1 and A 2 . A 1 starts with 100$ and A 2 starts with 150$ . The account value is allowed to be negative, as long as the sum of the two accounts is nonnegative: A + A >= 0 . T and T each attempt to withdraw 200$

> Lệch ghi (write skew) xảy ra khi từng giao dịch riêng lẻ đều tôn trọng các bất biến được yêu cầu, nhưng tổ hợp của chúng lại không thỏa mãn các bất biến đó. Ví dụ, các giao dịch T1 và T2 sửa đổi giá trị của hai tài khoản A1 và A2 . A1 khởi đầu với 100$ và A2 khởi đầu với 150$ . Giá trị tài khoản được phép âm, miễn là tổng của hai tài khoản không âm: A1 + A2 >= 0 . T1 và T2 mỗi giao dịch đều cố rút 200$
...
250$ is available in total. Both transactions assume they’re preserving the invariant and are allowed to commit. After the commit, A 1 has -100$ and A 2 has -50$ , which clearly violates the requirement to keep a sum of the accounts positive [FEKETE04] .

> tổng cộng chỉ có 250$ khả dụng. Cả hai giao dịch đều cho rằng mình đang bảo toàn bất biến và đều được phép commit. Sau khi commit, A1 có -100$ và A2 có -50$ , điều này rõ ràng vi phạm yêu cầu giữ cho tổng của các tài khoản là dương [FEKETE04] .
...
If two transactions attempt to modify the same value, only one of them is allowed to commit. This precludes a lost update anomaly. For example, transactions T 1 and T 2 both attempt to modify V . They read the current value of V from the snapshot that contains changes from all transactions that were committed before they started. Whichever transaction attempts to commit first, will commit, and the other one will have to abort. The failed transactions will retry instead of overwriting the value.

> Nếu hai giao dịch cùng cố sửa đổi một giá trị, chỉ một trong hai được phép commit. Điều này loại trừ dị thường mất cập nhật. Ví dụ, các giao dịch T1 và T2 đều cố sửa đổi V . Chúng đọc giá trị hiện tại của V từ ảnh chụp chứa các thay đổi của mọi giao dịch đã commit trước khi chúng bắt đầu. Giao dịch nào cố commit trước sẽ commit được, còn giao dịch kia sẽ phải abort. Các giao dịch thất bại sẽ thử lại thay vì ghi đè giá trị.
...
Backward-oriented concurrency control ensures that for any pair of transactions T 1 and T 2 , the following properties hold:

> Điều khiển tương tranh hướng-lùi bảo đảm rằng với bất kỳ cặp giao dịch T1 và T2 nào, các tính chất sau đều đúng:

- T was committed before the read phase of T began, so T is allowed to commit.

> - T1 đã được commit trước khi pha đọc của T2 bắt đầu, nên T2 được phép commit.
...
sect with the T 2 read set. In other words, T 1 hasn’t written any values T 2 should have seen.

> với tập đọc của T2 . Nói cách khác, T1 chưa hề ghi bất kỳ giá trị nào mà T2 lẽ ra phải thấy.

- The read phase of T 1 has completed before the read phase of T 2 , and the write set of T doesn’t intersect with the read or write sets of T . In other words,

> - Pha đọc của T1 đã hoàn tất trước pha đọc của T2 , và tập ghi của T1 không giao với tập đọc hay tập ghi của T2 . Nói cách khác,
...
released, while T 2 holds lock L 2 and waits for L 1 to be released.
...
T 2 which, in turn, waits for T 1 . Deadlock detection can be done periodically (once per time interval) or continuously (every time the waits-for graph is updated) [WEI‐ KUM01] . One of the transactions (usually, the one that attempted to acquire the lock more recently) is aborted.

> T2 , mà đến lượt mình, T2 lại chờ T1 . Việc phát hiện bế tắc có thể được thực hiện định kỳ (mỗi khoảng thời gian một lần) hoặc liên tục (mỗi khi đồ thị chờ-cho được cập nhật) [WEI‐ KUM01] . Một trong các giao dịch (thường là giao dịch đã cố giành khóa gần đây hơn) sẽ bị abort.
...
Wait-die T 1 is allowed to block and wait for the lock. Otherwise, T 1 is aborted and restarted. In other words, a transaction can be blocked only by a transaction with a higher timestamp.

> Wait-die T1 được phép chặn và chờ khóa. Ngược lại, T1 bị abort và khởi động lại. Nói cách khác, một giao dịch chỉ có thể bị chặn bởi một giao dịch có dấu thời gian cao hơn.
...
T 1 is allowed to wait. In other words, a transaction can be blocked only by a transaction with a lower timestamp.

> T1 được phép chờ. Nói cách khác, một giao dịch chỉ có thể bị chặn bởi một giao dịch có dấu thời gian thấp hơn.
...
> - Đọc đồng thời , khi nhiều luồng cùng truy cập một trang mà không sửa đổi nó.
> - Cập nhật đồng thời , khi nhiều luồng cùng cố thực hiện sửa đổi trên cùng một trang.
> - Đọc trong lúc ghi , khi một luồng đang cố sửa đổi nội dung trang, còn luồng kia lại đang cố truy cập chính trang đó để đọc.
